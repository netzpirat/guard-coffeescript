...
There following options can be passed to the CoffeeSCript Guard:
    ...
    :directories => false               # Do not create nested output directories

The guard detects by default nested directories and creates these within the output folder. This behaviour can
be switched off by passing the option :directories => false to the guard.
